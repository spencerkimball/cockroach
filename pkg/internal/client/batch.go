...
// value passed in. If the value passed in is a roachpb.Value with a non-zero
// MVCC timestamp, an error will also be reported if the MVCC timestamp of the
// existing value for the key is not exactly equal to the roachpb.Value's MVCC
// timestamp.
//
// key can be either a byte slice or a string. value can be any key type, a
